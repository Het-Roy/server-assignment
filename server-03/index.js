...
app.get("/states/highest-gdp", (req, res) => {
  ...
  const highestGDP = sortedStates[0];
  if (!highestGDP) {
...
app.patch("/states/:id/literacy", (req, res) => {
  ...
  //  if (req.body.name) state.name = req.body.name; //if the req.body.name exists then state.name i.e. the obtained object is equall to req.body.name which is raw json file
...
app.patch("/states/:id", (req, res) => {
  ...
  const updatedField = {}; //Here we will create a empty object, here we wiil store those fields which were updated 

  if (req.body.id) {
    state.id = req.body.id;
    updatedField.id = state.id;
  }
  if (req.body.name) { 
    state.name = req.body.name;
    updatedField.name = state.name;
  }
  if (req.body.population){
    state.population = req.body.population;
    updatedField.population = state.population;
  }
  if (req.body.literacyRate){
    state.literacyRate = req.body.literacyRate;
    updatedField.literacyRate = state.literacyRate
  } 
  if (req.body.annualBudget){
    state.annualBudget = req.body.annualBudget;
    updatedField.annualBudget = state.annualBudget;
  } 
  if (req.body.gdp){
...
app.delete("/states/:id", (req, res) => {
  ...
  states.splice(index, 1); //index num = index & deleteCount = 1

  res.status(204).json({    //204 --> no message will be printed on the message log in Postman
...
app.delete("/states/low-literacy/:percentage", (req, res) => {
  const reqPer = Number(req.params.percentage);

  const length = states.length;
  
  const remStates = states.filter(s => s.literacyRate >= reqPer) //we will the states higher than the required lit. rate
  
  console.log(remStates.length) 
...
//   states = states.filter(s => s.literacyRate >= reqPer);
